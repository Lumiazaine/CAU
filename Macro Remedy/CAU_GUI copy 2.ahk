...
; Función para descargar la última versión del ejecutable usando COM
DownloadLatestVersion() {
    ...
    ; Se asume que el tag en GitHub es del tipo "v1.0.1"
    downloadUrl := "https://github.com/JUST3EXT/CAU/releases/download/v" latestVersion "/CAU_GUI.exe"
    ; Crear el objeto WinHttpRequest para poder seguir redirecciones y enviar cabeceras
    ...
    ; Verificar que se obtuvo una respuesta 200 OK
    if (http.Status != 200) {
        WriteError("Error al descargar. HTTP Status: " http.Status)
        return false
    }

    ; Guardar el contenido binario en el archivo temporal
    fileData := http.ResponseBody
    FileDelete, %tempFile%
    FileAppend, % fileData, %tempFile%
