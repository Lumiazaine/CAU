...
Write-Host "=== PRUEBA BASICA DEL SISTEMA ===" -ForegroundColor Cyan
Write-Host ""
...
Write-TestLog "Iniciando pruebas basicas" "INFO"
...
foreach ($Module in $ModulesToTest) {
    ...
    Write-TestLog "Probando modulo: $($Module.Name)" "INFO"
    ...
    try {
        ...
        Write-TestLog "Modulo $($Module.Name) cargado correctamente" "SUCCESS"
...
Write-Host ""
Write-TestLog "=== RESUMEN DE CARGA DE MODULOS ===" "INFO"
Write-TestLog "Modulos cargados: $($LoadedModules.Count)" "SUCCESS"
Write-TestLog "Modulos fallidos: $($FailedModules.Count)" "$(if ($FailedModules.Count -gt 0) { 'ERROR' } else { 'SUCCESS' })"
...
if ($FailedModules.Count -gt 0) {
    Write-Host "Modulos fallidos:" -ForegroundColor Red
...
if ("CSVValidation" -in $LoadedModules) {
    Write-TestLog "=== PRUEBA DE VALIDACION CSV ===" "INFO"
    
    $CSVPath = Join-Path $ScriptPath $TestCSV
    if (Test-Path $CSVPath) {
        try {
            $ValidationResult = Test-CSVFile -CSVPath $CSVPath
            Write-TestLog "Validacion CSV ejecutada" "SUCCESS"
            Write-TestLog "Filas validas: $($ValidationResult.ValidRows)/$($ValidationResult.TotalRows)" "INFO"
...
if ("SamAccountNameGenerator" -in $LoadedModules) {
    ...
    $TestNames = @(
        @{ Name = "Juan"; Surname = "Garcia Lopez" },
        @{ Name = "Maria Luisa"; Surname = "Rodriguez Martin" }
    )
...
if ($FailedModules.Count -eq 0) {
    ...
    Write-TestLog "El sistema esta listo para pruebas" "SUCCESS"
} else {
