...
function Search-UsersInDomains {
    ...
    foreach ($Domain in $Domains) {
        ...
        try {
            ...
            if ($Users) {
                # Asegurar que $Users es un array para obtener el conteo correctamente
                $UserArray = @($Users)
                ...
                Write-Log "Dominio $($Domain.Name): $UserCount usuarios encontrados"
            } else {
...
function Show-SearchResults {
    ...
    Write-Host "`n=== RESULTADOS DE BUSQUEDA ($UserCount usuarios) ===" -ForegroundColor Green
    ...
    for ($i = 0; $i -lt $UserCount; $i++) {
        ...
        Write-Host "     Telefono: $(Get-SafePropertyValue $User.telephoneNumber)" -ForegroundColor Gray
        ...
        Write-Host "     Titulo: $(Get-SafePropertyValue $User.Title)" -ForegroundColor Gray
        Write-Host "     Ultimo acceso: $(Get-SafePropertyValue $User.LastLogonDate)" -ForegroundColor Gray
        Write-Host ""
    }
    
    Write-Host ("=" * 100) -ForegroundColor Green
    
    $Selection = Read-Host "Seleccione un usuario (1-$UserCount) o Enter para nueva busqueda"
...
function Show-CompleteUserInfo {
    ...
    Write-Host "  Email: $(Get-SafePropertyValue $User.mail)" -ForegroundColor Gray
    Write-Host "  Telefono: $(Get-SafePropertyValue $User.telephoneNumber)" -ForegroundColor Gray
    Write-Host "  Movil: $(Get-SafePropertyValue $User.mobile)" -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host "  Ultimo acceso: $(Get-SafePropertyValue $User.LastLogonDate)" -ForegroundColor Gray
    Write-Host "  Fecha de creacion: $(Get-SafePropertyValue $User.Created)" -ForegroundColor Gray
    Write-Host "  Ultima modificacion: $(Get-SafePropertyValue $User.Modified)" -ForegroundColor Gray
    Write-Host ""
    Write-Host "Otros:" -ForegroundColor Yellow
    Write-Host "  Descripcion: $(Get-SafePropertyValue $User.Description)" -ForegroundColor Gray
